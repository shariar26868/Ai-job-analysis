...
class QuoteService:
    ...
    def calculate_quote_for_worker(
        ...
        # Enforce minimum charge: if total < minimum_charge, use minimum_charge as final total
        minimum_charge = worker.minimum_charge or 0.0
        if minimum_charge > 0 and totalQuote < minimum_charge:
            print(f"⚡ Minimum charge applied for {worker.name}: £{totalQuote} → £{minimum_charge}")
